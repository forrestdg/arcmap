...
set FILES=1-1-2024-1-3-2004.csv
set ALPHAS=0.4
set TMAXES=60 90 120
FOR %%f in (%FILES%) do (
    "C:\Python26\ArcGIS10.0\python.exe" ..\scripts\import_randstad_scenario.py %%f
    ...
    FOR %%a in (%ALPHAS%) DO (
	    ...
	    move ..\Data\Randstad\jobs.csv ..\data\Randstad\results\jobs_scenario__%%t_%%a_%%f
        move ..\Data\Randstad\access.csv ..\data\Randstad\results\access_scenario__%%t_%%a_%%f
	)
    )
