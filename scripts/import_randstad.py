...
def select_input():
    data_path = os.path.join(os.path.abspath(os.path.dirname(__file__)),
                             "..","Data","Randstad")
    csv_full_paths = glob.glob(os.path.join(data_path, "*.csv")) 
    csvs = [ os.path.split(f)[1] for f in csv_full_paths ]
    csvs = [ f for f in csvs if f not in ('access.csv','jobs.csv') ]
...
def main():
    ...
    #
    csv_reader = csv.reader(open(os.path.join(data_path, input_csv)))
